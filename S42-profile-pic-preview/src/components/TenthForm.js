...
function TenthForm() {
  ...
  let calculateTotalMarks = () => {
    ...
    if (
      ...
      resultParaRef.current.innerHTML =
        "Congratulations! Your are passed in all subjects. <br/>";
    } else {
      resultParaRef.current.innerHTML = `<br/>Your are failed in (${failedSubjects}) subjects. Better luck next time.<br/>`;
    }
  ...
  let calculatePercentage = () => {
    ...
    if (percentage >= 70) {
      ...
      overAllGrade = "Frist Class";
    } else if (percentage >= 50 && percentage <= 60) {
      overAllGrade = "Second Class";
    } else if (percentage >= 40 && percentage <= 50) {
      overAllGrade = "Third Class";
    } else {
      overAllGrade = "Just Pass";
    }

    percentageParaRef.current.innerHTML = `Percentage : ${percentage.toFixed(
      0
    )}% and grade - ${overAllGrade} `;
  };
  ...
  // profile pic 
  let [profile, setProfile] = useState('./images/noImage.jpg');

  return (
    <div>
      <form>
      <div>
          <img className="profile" src={profile} alt="noImageIsAvaliable" />
          <br />
        ...
        <div>
          ...
          <select
            id="marital-status"
            name="marital-status"
            onChange={(eventObject) => handleMaritalStatusChange(eventObject)}>
        ...
        <div className="languagesCheckBox">
          <label style={{ width: "190px", textAlign: "end" }}>
            Languages Known:
          </label>
          <input
            type="checkbox"
            value="english"
            onChange={(eventObject) => {
              languagesKnown.eng = eventObject.target.checked;
            }}
          />
          <label>English</label>
          <input
            type="checkbox"
            onChange={(eventObject) => {
              languagesKnown.tel = eventObject.target.checked;
            }}
          />
          <label>Telugu</label>
          <input
            type="checkbox"
            onChange={(eventObject) => {
              languagesKnown.hin = eventObject.target.checked;
            }}
          />
          <label>Hindi</label>
          <input
            type="checkbox"
            onChange={(eventObject) => {
              languagesKnown.tam = eventObject.target.checked;
            }}
          />
          <label>Tamil</label>
          <input
            type="checkbox"
            onChange={(eventObject) => {
              languagesKnown.kan = eventObject.target.checked;
            }}
          />
          <label>Kanada</label>
        </div>
